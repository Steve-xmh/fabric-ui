...
fn box_blur_vert(
    ...
    for i in 0..width {
        ...
        let mut val_a: isize = (blur_radius as isize + 1) * isize::from(fv);
        ...
        if blur_radius > height {
            val_a += (blur_radius - height) as isize * isize::from(lv);
        }

        for _ in 0..min(height, blur_radius + 1) {
            let bb = get_bottom(ri);
            ri += width;
            val_a += isize::from(bb) - isize::from(fv);

            frontbuf[ti] = round(val_a as f32 * iarr) as u8;
            ti += width;
        }
        ...
        if height > blur_radius {
            ...
            for _ in (blur_radius + 1)..(height - blur_radius) {
                ...
                val_a += isize::from(bb1) - isize::from(bb2);
            ...
            for _ in 0..min(height - blur_radius - 1, blur_radius) {
                ...
                val_a += isize::from(lv) - isize::from(bb);
